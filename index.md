## Welcome to my site :) 

<img src="https://user-images.githubusercontent.com/91928008/189604764-0ca06149-dc89-46d7-9e52-e5fa3bb23ed1.png" width="50" height="50"/>
I am sorry for all this but it still under construction!

### About me

I am a UBC student pursuing a major in computer science and minor in data science. I have always had a keen interest in math, economics, physics and other quantitative subjects. I am continuing my interest in quantitative fields to break into a career in tech. 

I am a natural student (passionate about learning new things) and I have tried to incorporate most of it in my projects listed below

- **A course management system schedules** your homework and keeps track with your academic progress with a functional GUI in  _java_    [link](https://github.com/PUSH-YA/CourseManagementApp.git)
- **A knn classification model** for predicting wine quality scores based on its chemical properties in _R_   [link](https://github.com/kellestia/dsci100_group_project.git)
- **A tic toe game player** played through console and using 2D arrays in _python_
- **Pong game** which is a multiplayer game using java's awt and swing libraries in _java_
- **A lab time scheduler** which will assign ta's to lab slots using a generational recursive algorithm 


### Currently working on

I am currently trying to further learn about data structures and algorithms in C++, completing Google data analytics certificate and learning data science fundamentals in python

### Contact
[my resume](https://drive.google.com/file/d/1-gQchlAUzPYZRY34emxfhDWYqG1WYoSh/view?usp=sharing)

You can find me on [github](https://github.com/PUSH-YA) or [linkedin](https://www.linkedin.com/in/pushya-jain-4546a6150/) or get in touch with me directly on my [email](mailto:pushya22.jain@gmail.com)
